using AsepriteLoader.FileFormats.Chunks;
...
public class Frame : IBinaryReadable<Frame>
{
	...
	public static Frame ReadBinary(BinaryReader reader)
	{
		...
		for (var i = 0; i < ret.Header.TheNumberOfChunks; ++i)
		{
			var chunkHeader = ChunkHeader.ReadBinary(reader);
	...
	public Rgba32[] GetPaletteColors()
	{
		...
		if (OldPalette04Chunks.Count != 0)
			return OldPalette04Chunks[0].GetPaletteColors();
		if (OldPalette11Chunks.Count != 0)
